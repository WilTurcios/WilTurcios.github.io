const golden_state_warriors_association = [
  {
    id: 225,
    productName: "Golden State Warriors, association 2022 - 2023",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2022-23-association-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 226,
    productName: "Golden State Warriors, association 2021 - 2022",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2021-22-association-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 228,
    productName: "Golden State Warriors, association 2017 - 2019",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2017-19-association-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
];
const golden_state_warriors_icon = [
  {
    id: 231,
    productName: "Golden State Warriors, icon 2022 - 2023",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2022-23-icon-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 233,
    productName: "Golden State Warriors, icon 2021 - 2022",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2021-22-icon-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 235,
    productName: "Golden State Warriors, icon 2017 - 2019",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2017-19-icon-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
];
const golden_state_warriors_statement = [
  {
    id: 236,
    productName: "Golden State Warriors, statement 2022 - 2023",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2022-23-statement-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 238,
    productName: "Golden State Warriors, statement 2019 - 2020",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2019-20-statement-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 241,
    productName: "Golden State Warriors, statement 2017 - 2019",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2017-19-statement-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
];
const golden_state_warriors_city = [
  {
    id: 242,
    productName: "Golden State Warriors, city 2022 - 2023",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2022-23-city-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 243,
    productName: "Golden State Warriors, city 2020 - 2021",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2020-21-city-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 244,
    productName: "Golden State Warriors, city 2019 - 2020",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2019-20-city-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 245,
    productName: "Golden State Warriors, city 2021 - 2022",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2021-22-city-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 246,
    productName: "Golden State Warriors, city 2018 - 2019",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2018-19-city-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
];
const golden_state_warriors_classic = [
  {
    id: 248,
    productName: "Golden State Warriors, classic 2022 - 2023",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2022-23-classic-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 250,
    productName: "Golden State Warriors, classic 2019 - 2020",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2019-20-classic-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
  {
    id: 251,
    productName: "Golden State Warriors, classic 2021 - 2022",
    productDescription:
      "Producto de alta calidad, con la mejor tela, duradero etc etc etc, super caro tambien, no se que mas poner.",
    imgUrl:
      "../assets/golden_state_w/golden-state-warriors-2021-22-classic-jersey.jpg",
    sizes: ["S", "M", "L", "XL", "XXL"],
    pricing: {
      S: 12,
      M: 12.5,
      L: 14,
      XL: 16,
      XXL: 17,
    },
    stock: 20,
    category: "Golden State Warriors",
    productKeywords:
      "Product, Golden State Warriors, blanca, fútbol, futbol soccer hombre masculino",
    generalCategory: "Futbol",
    rating: 4.59,
  },
];

const GOLDEN_STATE_WARRIORS = [
  ...golden_state_warriors_association,
  ...golden_state_warriors_city,
  ...golden_state_warriors_classic,
  ...golden_state_warriors_icon,
  ...golden_state_warriors_statement,
];

export default GOLDEN_STATE_WARRIORS;
